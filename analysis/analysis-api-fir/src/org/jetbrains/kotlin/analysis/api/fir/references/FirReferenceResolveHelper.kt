...
internal object FirReferenceResolveHelper {
    ...
    internal fun resolveSimpleNameReference(
        ...
        val fir = when (val baseFir = adjustedResolutionExpression.getOrBuildFir(analysisSession.llResolutionFacade)) {
            is FirSmartCastExpression -> baseFir.originalExpression
            else -> baseFir
        }
        val session = analysisSession.llResolutionFacade.useSiteFirSession
        return when (fir) {
    ...
    private fun getSymbolByResolvedNameReference(
        ...
        if (parentAsCall != null) {
            val firCall = parentAsCall.getOrBuildFir(analysisSession.llResolutionFacade)?.unwrapSafeCall()
    ...
    private fun getSymbolsByNameArgumentExpression(
        ...
        val firCall = ktCallExpression.getOrBuildFir(analysisSession.llResolutionFacade)?.unwrapSafeCall() as? FirCall ?: return emptyList()
    ...
    private fun handleUnknownFirElement(
        ...
        while (parent != null) {
            ...
            val parentFir = selectorExpression.getOrBuildFir(analysisSession.llResolutionFacade)
