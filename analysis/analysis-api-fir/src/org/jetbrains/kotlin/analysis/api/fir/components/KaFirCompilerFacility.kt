    ...
    private fun compileChunk(
        ...
        val evaluatorData = when (chunk.kind) {
            ChunkKind.MAIN if codeFragmentMappings != null -> {
                ...
                val localDeclarationsLoweringData = contextDeclarationCache?.localDeclarationsLoweringData
                    .orEmpty()
                    .toMutableMap()

                JvmEvaluatorData(
                    localDeclarationsLoweringData = localDeclarationsLoweringData,
...
private class ContextDeclarationCache(private val contextDeclaration: KtDeclaration) {
     ...
     */
    var localDeclarationsLoweringData: Map<IrFunction, JvmBackendContext.LocalFunctionData>? = null
        private set
    ...
    fun initialize(fir2IrResult: Fir2IrActualizedResult, commonMemberStorage: Fir2IrCommonMemberStorage, evaluatorData: JvmEvaluatorData?) {
        require(localDeclarationsLoweringData == null && customCommonMemberStorage == null) { "Cache is already initialized" }

        localDeclarationsLoweringData = evaluatorData?.localDeclarationsLoweringData
