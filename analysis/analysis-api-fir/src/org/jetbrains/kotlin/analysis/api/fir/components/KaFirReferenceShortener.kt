...
private class FirTowerDataContextProvider private constructor(
    ...
    companion object {
        fun create(llResolutionFacade: LLResolutionFacade, targetElement: KtElement): FirTowerDataContextProvider {
            val firFile = targetElement.containingKtFile.getOrBuildFirFile(llResolutionFacade)

            val sessionHolder = run {
                val firSession = llResolutionFacade.useSiteFirSession
...
private class ElementsToShortenCollector(
    ...
    private fun shortenClassifierIfAlreadyImported(
        ...
        if (shorteningContext.findPropertiesInScopes(scopes, name).isNotEmpty()) {
            val firForElement = element.getOrBuildFir(llResolutionFacade) as? FirQualifiedAccessExpression
    ...
    private fun importAffectsUsagesOfClassesWithSameName(classToImport: ClassId, importAllInParent: Boolean): Boolean {
        ...
        containingFile.accept(object : KtVisitorVoid() {
            ...
            override fun visitSimpleNameExpression(expression: KtSimpleNameExpression) {
                ...
                val contextProvider = FirTowerDataContextProvider.create(llResolutionFacade, expression)
    ...
    private fun resolveUnqualifiedAccess(
        ...
        val candidates = AllCandidatesResolver(shorteningContext.analysisSession.firSession).getAllCandidates(
            llResolutionFacade, fakeFirQualifiedAccess, name, expressionInScope, ResolutionMode.ContextIndependent,
        )
    ...
    private fun findCandidatesForPropertyAccess(
        ...
        val candidates = AllCandidatesResolver(shorteningContext.analysisSession.firSession).getAllCandidates(
            llResolutionFacade, fakeFirQualifiedAccess, name, elementInScope, ResolutionMode.ContextIndependent,
        )
        return candidates.filter { overloadCandidate ->
    ...
    private fun KtExpression.isCompanionMemberUsedForEnumEntryInit(resolvedSymbol: FirCallableSymbol<*>): Boolean {
        ...
        val firEnumEntry = enumEntry.resolveToFirSymbol(llResolutionFacade) as? FirEnumEntrySymbol ?: return false
