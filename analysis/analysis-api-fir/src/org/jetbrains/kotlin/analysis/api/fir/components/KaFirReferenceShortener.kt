...
internal class KaFirReferenceShortener(
    ...
    override fun collectPossibleReferenceShortenings(
        ...
        val towerContext = FirTowerDataContextProvider.create(llResolutionFacade, declarationToVisit)
    ...
    private fun KtElement.getCorrespondingFirElement(): FirElement? {
        ...
        val firElement = getOrBuildFir(llResolutionFacade)
...
private class FirTowerDataContextProvider private constructor(
    ...
    companion object {
        fun create(firResolutionFacade: LLResolutionFacade, targetElement: KtElement): FirTowerDataContextProvider {
            val firFile = targetElement.containingKtFile.getOrBuildFirFile(firResolutionFacade)

            val sessionHolder = run {
                val firSession = firResolutionFacade.useSiteFirSession
                val scopeSession = firResolutionFacade.getScopeSessionFor(firSession)
...
private class FirShorteningContext(val analysisSession: KaFirSession) {
    ...
    private val firSession: FirSession
        get() = llResolutionFacade.useSiteFirSession
...
private class ElementsToShortenCollector(
    ...
    private val firResolutionFacade: LLResolutionFacade,
) {
    ...
    private fun shortenClassifierIfAlreadyImported(
        ...
        if (shorteningContext.findPropertiesInScopes(scopes, name).isNotEmpty()) {
            val firForElement = element.getOrBuildFir(firResolutionFacade) as? FirQualifiedAccessExpression
    ...
    private fun importAffectsUsagesOfClassesWithSameName(classToImport: ClassId, importAllInParent: Boolean): Boolean {
        ...
        containingFile.accept(object : KtVisitorVoid() {
            ...
            override fun visitSimpleNameExpression(expression: KtSimpleNameExpression) {
                ...
                val contextProvider = FirTowerDataContextProvider.create(firResolutionFacade, expression)
    ...
    private fun resolveUnqualifiedAccess(
        ...
        val candidates = AllCandidatesResolver(shorteningContext.analysisSession.firSession).getAllCandidates(
            firResolutionFacade, fakeFirQualifiedAccess, name, expressionInScope, ResolutionMode.ContextIndependent,
        )
    ...
    private fun findCandidatesForPropertyAccess(
        ...
        val candidates = AllCandidatesResolver(shorteningContext.analysisSession.firSession).getAllCandidates(
            firResolutionFacade, fakeFirQualifiedAccess, name, elementInScope, ResolutionMode.ContextIndependent,
        )
        return candidates.filter { overloadCandidate ->
    ...
    private fun KtExpression.isCompanionMemberUsedForEnumEntryInit(resolvedSymbol: FirCallableSymbol<*>): Boolean {
        ...
        val firEnumEntry = enumEntry.resolveToFirSymbol(firResolutionFacade) as? FirEnumEntrySymbol ?: return false
