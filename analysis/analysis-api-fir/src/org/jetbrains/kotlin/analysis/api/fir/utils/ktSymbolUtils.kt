...
internal fun FirBasedSymbol<*>.getContainingKtModule(firResolutionFacade: LLResolutionFacade): KaModule {
    val target = when (this) {
        is FirCallableSymbol -> {
            // callable fake overrides have use-site FirModuleData
            dispatchReceiverClassLookupTagOrNull()?.toRegularClassSymbol(firResolutionFacade.useSiteFirSession) ?: this
        }
        else -> this
    }
...
internal fun KaSymbol.getContainingKtModule(firResolutionFacade: LLResolutionFacade): KaModule = when (this) {
    is KaFirSymbol<*> -> firSymbol.getContainingKtModule(firResolutionFacade)
    is KaReceiverParameterSymbol -> owningCallableSymbol.getContainingKtModule(firResolutionFacade)
