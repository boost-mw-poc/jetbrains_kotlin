...
internal class KaFirResolver(
    ...
    override fun KtReference.isImplicitReferenceToCompanion(): Boolean = withValidityAssertion {
        ...
        val implicitInvokeCall = run {
            ...
            parentCallExpression?.getOrBuildFir(analysisSession.llResolutionFacade) as? FirImplicitInvokeCall
        }

        val wholeQualifier = implicitInvokeCall?.explicitReceiver
            ?: element.getOrBuildFir(analysisSession.llResolutionFacade)
    ...
    private inline fun <T> getCallInfo(
        ...
        val psiToResolve = containingCallExpressionForCalleeExpression
            ...
            ?: psi
        val fir = psiToResolve.getOrBuildFir(analysisSession.llResolutionFacade) ?: return emptyList()
    ...
    private fun KtElement.getContainingBinaryExpressionForIncompleteLhs(): KtBinaryExpression? {
        ...
        val firBinaryExpression = binaryExpression.getOrBuildFir(analysisSession.llResolutionFacade)
    ...
    private fun getInitializerOfReferencedLocalVariable(variableReference: FirExpression): FirFunctionCall? {
        return variableReference.toReference(llResolutionFacade.useSiteFirSession)
    ...
    private fun FirQualifiedAccessExpression.collectCallCandidates(
        ...
        val candidates = AllCandidatesResolver(analysisSession.firSession).getAllCandidates(
            analysisSession.llResolutionFacade,
    ...
    private fun FirDelegatedConstructorCall.collectCallCandidatesForDelegatedConstructorCall(
        ...
        val derivedClass = findDerivedClass(psi)?.resolveToFirSymbolOfTypeSafe<FirClassSymbol<*>>(llResolutionFacade) ?: return emptyList()

        val candidates = AllCandidatesResolver(analysisSession.firSession)
            .getAllCandidatesForDelegatedConstructor(analysisSession.llResolutionFacade, this, derivedClass.toLookupTag(), psi)
    ...
    private inline fun <R> wrapError(element: KtElement, action: () -> R): R {
        return try {
            ...
            rethrowExceptionWithDetails(
                ...
                element.getOrBuildFir(llResolutionFacade)?.let { withFirEntry("fir", it) }
            }
        }
