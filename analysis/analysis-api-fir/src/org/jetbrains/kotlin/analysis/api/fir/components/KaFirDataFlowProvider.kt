...
internal class KaFirDataFlowProvider(
    ...
    private fun getMatchingFirExpressionWithSmartCast(expression: KtExpression): FirSmartCastExpression? {
        ...
        return when (val firExpression = possibleFunctionCall.getOrBuildFir(analysisSession.llResolutionFacade)) {
    ...
    private fun getMatchingFirQualifiedAccessExpression(expression: KtExpression): FirQualifiedAccessExpression? {
        ...
        return when (val firExpression = wholeExpression.getOrBuildFir(analysisSession.llResolutionFacade)) {
    ...
    private fun computeStatements(statements: List<KtExpression>): List<FirElement> {
        ...
        val firStatements = statements.map { it.unwrap().getOrBuildFirOfType<FirElement>(llResolutionFacade) }
    ...
    private fun computeCommonParent(statements: List<KtElement>): FirElement {
        ...
        val firContainer = collectUseSiteContainers(parent, llResolutionFacade)?.lastOrNull()
        ...
        return parent.parentsWithSelf
            ...
            .firstNotNullOf { it.getOrBuildFir(llResolutionFacade) }
    }
    ...
    private fun computeOperationDefaultType(defaultStatement: KtExpression): ConeKotlinType? {
        ...
        val expressionCall = binaryExpression.getOrBuildFirSafe<FirQualifiedAccessExpression>(llResolutionFacade) ?: return null
    ...
    private fun findControlFlowGraph(anchor: KtElement, firStatements: List<FirElement>): ControlFlowGraph? {
        ...
        for (parentDeclaration in parentDeclarations) {
            val parentFirDeclaration = parentDeclaration.resolveToFirSymbol(llResolutionFacade, FirResolvePhase.BODY_RESOLVE).fir
