...
abstract class AbstractByQualifiedNameLazyDeclarationResolveTest : AbstractFirLazyDeclarationResolveOverAllPhasesTest() {
    ...
    override fun doTestByMainModuleAndOptionalMainFile(mainFile: KtFile?, mainModule: KtTestModule, testServices: TestServices) {
        ...
        val resolutionFacade = LLFirResolveSessionService.getInstance(psiFile.project).getLLResolutionFacade(mainModule.ktModule)
    ...
    private fun findRegularClass(classId: ClassId, module: KaModule, llResolutionFacade: LLResolutionFacade): FirRegularClassSymbol {
        return llResolutionFacade.getSessionFor(module).getRegularClassSymbolByClassId(classId) ?: error("'$classId' is not found")
    }
