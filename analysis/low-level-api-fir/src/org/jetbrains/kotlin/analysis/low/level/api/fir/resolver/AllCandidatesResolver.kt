...
class AllCandidatesResolver(private val firSession: FirSession) {
    ...
    fun getAllCandidates(
        ...
        initializeBodyResolveContext(llResolutionFacade, element)
    ...
    private fun initializeBodyResolveContext(llResolutionFacade: LLResolutionFacade, element: KtElement) {
        val firFile = element.containingKtFile.getOrBuildFirFile(llResolutionFacade)
        ...
        val containingDeclarations =
            element.parentsOfType<KtDeclaration>().map { it.resolveToFirSymbol(llResolutionFacade).fir }.toList().asReversed()
